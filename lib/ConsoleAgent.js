...
ConsoleAgent.prototype.injection = function(require, debug, options) {
  ...
  function unwrapConsole() {
    ...
    debug.command('Console.clearMessages');
    Object.keys(console).forEach(function(fn) {
      if (!/^_/.test(fn)) {
  ...
  function sendMessageToInspector(fn, wrapper, args) {
    debug.emitEvent('Console.messageAdded', {
      message: new ConsoleMessage(fn, wrapper, args)
    });
  }

  debug.registerAgentEvent(
    'Console.messageAdded',
    function(args, response, injectedScript, DebuggerScript) {
      var message = response.body.message;
      message.parameters = message.parameters.map(function(arg) {
        return injectedScript.wrapObject(arg, 'console');
      });
    });
  ...
  debug.registerAgentCommand(
    ...
    function(args, response, injectedScript, DebuggerScript) {
      injectedScript.releaseObjectGroup('console');
      debug.emitEvent('Console.messagesCleared');
    });

  debug.registerAgentEvent('Console.messagesCleared');
