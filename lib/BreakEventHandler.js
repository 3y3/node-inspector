 ...
 * @constructor
 */
function BreakEventHandler(frontendClient, debuggerClient, scriptManager, consoleBridge) {
  this._frontendClient = frontendClient;
  this._debuggerClient = debuggerClient;
  this._scriptManager = scriptManager;
  this._consoleBridge = consoleBridge;
...
BreakEventHandler.prototype._onBreak = function(obj) {
  ...
  // When debugger was connected we wrap native console functions
  // with breakpoint (see ./lib/ConsoleBridge._createBreakWrapper).
  // If break event fired in wrapper we need to finish injection
  // and resume the execution
  if (!this._consoleBridge.injected) {
    var isWrapper = this._consoleBridge.isBreakWrapper(obj.sourceLineText);

    if (isWrapper) {
      this._consoleBridge.injectClient();
      return;
    }
  }
