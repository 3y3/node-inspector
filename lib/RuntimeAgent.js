...
RuntimeAgent.prototype = {
  enable: function(params, done) {
    ...
    var sendMainContext = function() {
      this._frontendClient.sendEvent('Runtime.executionContextCreated', {
        context: {
          id: this._debuggerClient.target.pid,
          isPageContext: true,
          name: '<main context>',
          origin: 'node-inspector'
        }
      });
    }.bind(this);

    if (this._debuggerClient.target) {
      process.nextTick(sendMainContext);
    } else {
      this._debuggerClient.on('connect', sendMainContext);
    }
  },
