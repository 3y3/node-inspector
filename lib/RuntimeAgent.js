 ...
 * @constructor
 */
function RuntimeAgent(debuggerClient, consoleBridge) {
...
RuntimeAgent.prototype = {
  ...
  getProperties: function(params, done) {
    ...
    if (this._callFramesProvider.isScopeId(params.objectId)) {
      ...
      if (!params.ownProperties || params.accessorPropertiesOnly) {
        ...
        if (this._consoleBridge.isConsoleId(params.objectId)) {
          this._getPropertiesOfConsoleId(params.objectId, done);
        }
        else {
  ...
  _getPropertiesOfConsoleId: function(objectId, done) {
    var messageNum = objectId.split(':')[1];
    this._consoleBridge.getMessageScope(
      messageNum,
      function(error, responseRefs) {
        if (error) {
          return done(error);
        }

        var obj = responseRefs[objectId],
            props = convert.v8ObjectToInspectorProperties(obj, responseRefs);
